...
export const updateMediaNotification = async (song, isPlaying = true) => {
    ...
    try {
        ...
        await Notifications.scheduleNotificationAsync({
            ...
            content: { title, body, sound: null, sticky: true, autoDismiss: false, color: '#1ed760', priority: Notifications.AndroidNotificationPriority.MAX, data: { songId: song.id, isPlaying } },
            trigger: null,
        });
    } catch (_) {}
};
...
export const dismissMediaNotification = async () => {
    ...
    try { await Notifications.dismissNotificationAsync(NOTIFICATION_ID); } catch (_) {}
};
